import { AnySelectMenuInteraction, ClientEvents, ContextMenuCommandInteraction } from "discord.js";
...
export default class menuHandler extends baseDiscordEvent {
    public name: keyof ClientEvents = "interactionCreate";
    public once: boolean = false;
   // @ts-ignore
    async executeEvent(interaction: AnySelectMenuInteraction | ContextMenuCommandInteraction<"cached">):Promise<any> {
        if ((!interaction.isContextMenuCommand() && !interaction.isAnySelectMenu()) || !this.client.ready) return;
        const type = interaction.isContextMenuCommand() ? "contextMenu" : "menu";
        const name = interaction.isContextMenuCommand() ? interaction.commandName.toLowerCase() : interaction.customId.toLowerCase();
        const command:menuCommand = this.client.menus.get(name) || this.client.menus.find(e => e.type === type && e.passFilter(interaction)) 
        if (!command) return;
