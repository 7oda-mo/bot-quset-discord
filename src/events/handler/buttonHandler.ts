...
export default class buttonInteractionHandler extends baseDiscordEvent {
    ...
    async executeEvent(interaction: ButtonInteraction) {
        if (!interaction.isButton() || !this.client.ready) return;
        const command = this.client.buttons.get(interaction.customId.toLowerCase()) || this.client.buttons.find(e => e?.passFilter(interaction))
        if (!command) return;
        ...
        try {
            ...
            this.logger.error(`Failed to execute button command ${command.name} (${commandKey}) for user ${interaction.user.id} in guild ${interaction.guildId}:`, err);
